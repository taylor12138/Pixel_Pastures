...
func test_save_export_import() -> void:
	...
	farm_grid_manager.set_plot_state(Vector2i(7, 5), farm_grid_manager.PLOT_DRY_SOIL)
	farm_grid_manager.mark_tile_watered(Vector2i(7, 5))
	...
	_assert(farm_grid_manager.get_plot_state(Vector2i(7, 5)) == farm_grid_manager.PLOT_WET_SOIL, "导入恢复湿土地块状态")
	...
	_assert(farm_grid_manager.unlocked_plot_count == 20, "导入恢复 unlocked_plot_count")
